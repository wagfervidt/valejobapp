...
class jobApplication extends LCAPApplicationService {
    async init() {
        this.on('READ', 'JobRequisitionIntegrity', async (request, next) => {
            ...
            try {
                ...
                if (token) {
                    const urlJobApplication = 'https://api4.successfactors.com/odata/v2/JobApplication?$expand=jobRequisition/recruiter&$format=json';
        ...
        this.on('READ', 'PublicRelative', async (request, next) => {
            async function obterTokenAcesso() {
                const urlIdp = 'https://api4.successfactors.com/oauth/idp';
                const dataIdp = querystring.stringify({
                    client_id: 'ZmQ3ZGU4NjQwOTY4NTQ2N2M4N2VhMTY1MWViMQ',
            ...
            try {
                const token = await obterTokenAcesso();
                if (token) {
                    const urlJobApplication = 'https://api4.successfactors.com/odata/v2/JobApplication?$format=json';
                    const optionsJobApplication = {
                        method: 'GET',
                        headers: {
                            'Authorization': `Bearer ${token}`
